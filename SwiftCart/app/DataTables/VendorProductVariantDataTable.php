...
class VendorProductVariantDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        {
            $edit = '<a href="'.route("vendor.product_variant.edit",$query->id).'" class="btn btn-primary mb-1"><i class="fa-solid fa-pen-to-square"></i></a>';
            $delete = '<a onclick="delete_row(event)" href="'.route("vendor.product_variant.destroy",$query->id).'" id="' . $query->id . '"
            class="btn btn-danger mb-1 delete-item"><i class="fa-solid fa-trash"></i></a>';
            $variant_items = '<a
            href="'.route('vendor.product_variant_item.index', ['variant_id'=>$query->id]).'" id="' . $query->id . '"

    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                $delete = '<a onclick="delete_row(event)" href="' . route("admin.review.delete", $query->id) . '" id="' . $query->id . '" class="btn btn-danger delete-item"><i class="fa-solid fa-trash"></i></a>';
                return $delete;
            })
            ...
            ->addColumn('name', function ($query) {
                return '<a  href="'.route("product_details",$query->product->id).'" >' .
                    $query->product->name .
                    '</a>';
            })

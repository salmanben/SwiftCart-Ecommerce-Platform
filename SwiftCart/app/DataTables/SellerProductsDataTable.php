...
class SellerProductsDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            {
                return "<img width=80 height=60 src='".asset("storage/upload/".$query->image)."' >";
            })
            ...
            {
                $edit = '<a href="'.route("admin.product.edit",$query->id).'" class="btn btn-primary mb-1"><i class="fa-solid fa-pen-to-square"></i></a>';
                $delete = '<a onclick="delete_row(event)" href="'.route("admin.product.destroy",$query->id).'" id="' . $query->id . '"
                ...
                <div class="dropdown mb-1">
                   ...
                   <ul class="dropdown-menu" aria-labelledby="dropdownMenuLink">
                      <li><a class="dropdown-item" href="' . route('admin.product_image_gallery.index', ['id'=>$query->id]) . '"><i class="fa-solid fa-image me-2"></i> Image Gallery</a></li>
                      <li><a class="dropdown-item" href="' . route('admin.product_variant.index', ['id'=>$query->id]) . '"><i class="fa-sharp fa-solid fa-square-arrow-up-right me-2"></i>Variants</a></li>
                   </ul>
                </div>
              ';
            ...
            ->addColumn('approve', function($query)
            {
                ...
                {
                    return '<select class="form-select form-control shadow-none switch-approve" onchange="switch_approve(event)" name="approve" id="'.$query->id.'" aria-label="Default select example">
            ...
            ->rawColumns(['image', 'action', 'product_type', 'status', 'approve', 'vendor', 'price'])
            ->setRowId('id');
    }
    ...
    {
        if (request()->routeIs('admin.seller_pending_products'))
            return $model->
                       where('is_approved', 0)->where('vendor_id', '!=',auth()->user()->vendor->id)->newQuery();
        else
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('approve')->width(100)->addClass('text-center'),

...
class WithdrawMethodDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                $edit = '<a href="'.route("admin.withdraw_method.edit",$query->id).'" class="btn btn-primary mb-1"><i class="fa-solid fa-pen-to-square"></i></a>';
                $delete = '<a onclick="delete_row(event)" href="' . route("admin.withdraw_method.destroy", $query->id) . '" id="' . $query->id . '"
                    class="btn btn-danger mb-1 delete-item"><i class="fa-solid fa-trash"></i></a>';

                return $edit." ".$delete;
            })

            ->addColumn('min_amount', function ($query) use($currency_icon){
                return $currency_icon .   $query->min_amount;
            })
            ->addColumn('max_amount', function ($query) use($currency_icon) {
                return $currency_icon .   $query->max_amount;
            })
            ...
            ->rawColumns(['action', 'status', 'min_amount', 'max_amount', 'charge'])
            ->setRowId('id');
    }
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('min_amount')->addClass('text-center'),
            Column::make('max_amount')->addClass('text-center'),
            Column::make('charge')->addClass('text-center'),

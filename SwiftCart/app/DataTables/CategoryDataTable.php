...
class CategoryDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            {
                $edit = '<a href="'.route("admin.category.edit",$query->id).'" class="btn btn-primary mb-1"><i class="fa-solid fa-pen-to-square"></i></a>';
                $delete = '<a onclick="delete_row(event)" href="'.route("admin.category.destroy",$query->id).'" id="' . $query->id . '"
                class="btn btn-danger delete-item mb-1"><i class="fa-solid fa-trash"></i></a>';
                return $edit." ".$delete;
            })
            ->addColumn('icon', function($query)
            {
                return "<i style='font-size:30px;' class='".$query->icon." text-primary'></i>";
            })

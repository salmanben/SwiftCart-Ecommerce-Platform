...
class NewsletterSubscriberDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        {
            $delete = '<a onclick="delete_row(event)" href="'.route("admin.newsletter_subscribers.delete",$query->id).'" id="' . $query->id . '" style="margin-left:20px;"
            class="btn btn-danger delete-item"><i class="fa-solid fa-trash"></i></a>';
            return $delete;
        })
        ->addColumn('is_verified', function($query)
        {
            return $query->is_verified == 1 ?  "<span class='badge bg-success text-white'>Yes</span>" :   "<span class=' badge bg-danger text-white'>No</span>";
        })
